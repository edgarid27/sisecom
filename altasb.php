...
session_start();
    ...
    $shsq2 ="SELECT id, modelo FROM `mercancia` where estatus = 1  order by modelo";
    $rec2= mysqli_query($con,$shsq2);
    
    $shsq3 ="SELECT id, proveedor FROM `proveedor` where estatus = 1 order by proveedor";
    $rec3= mysqli_query($con,$shsq3);
    
    $shsq4 ="SELECT * FROM tmercancia WHERE estatus = 1";
    $rec4= mysqli_query($con,$shsq4);
    
    $shsq5 ="SELECT id, estado FROM `estado` where estatus = 1 and id != 4 order by estado";
    $rec5= mysqli_query($con,$shsq5);
...
<head>
    ...
    <!--<script src="js/mercanciacop.js"></script>-->
    <script src="js/altasbc.js"></script>
    ...
    <script type="text/javascript">
        function Redireccionar(){ 
            location.href ="php/pdf/genPDFAlta.php";
        };
    </script>
	...
	<title>Panel principal de administración de altas.</title>
</head>
...
<body>
      ...
      <table id="altas" class="table table-condensed table-hover table-striped" width="100%" cellspacing="0" >
            <thead>
                <tr>
                    ...
                    <th data-column-id="modelo">ID M.</th>
                    <th data-column-id="tipomercancia">Mercancia</th>
                    <th data-column-id="marcas">Marca</th>
                    <th data-column-id="model">Modelo</th>
                    <th data-column-id="nota">Nota</th>
                    <th data-column-id="proveedores">Proveedor</th>
                    <th data-column-id="estado">Estado</th>
                    <th data-column-id="cantidad">Cantidad</th>
                    <th data-column-id="catidadrestm">C.Exc.</th>
       ...
       <div class="col-md-12">
        	<button type="button" style="margin-left: 10px;" class="btn  pull-right clearfix btn-success" onclick="ExportToExcel('altas', 'W3C Example Table')" value="Export to Excel"><span class="fa fa-file-excel-o" style="margin-left: 10px;"></span> Exportar a PDF</button>
    ...
    <fieldset>
        <legend>Modificar alta</legend>
    </fieldset>
    <form id="formal">
        <div class="form-group">
            <label for="aid">ID de la baja:</label>
            <input type="text" class="form-control" id="aid" name="aid" placeholder="ID de la alta" readonly="readonly"/>
        </div>

        <div class="form-group">
            <label for="id1">ID de la mercancia:</label><br />
			<input type="text" class="form-control" id="mid" name="mid" placeholder="ID de la mercancia" readonly="readonly" />
		</div>
        
        <div class="form-group">
            <label for="cant">Cantidad de la mercancia:</label><br />
			<input type="numeric" class="form-control" id="mcant" name="mcant" placeholder="Cantidad de la mercancia" readonly="readonly" />
		</div>
        
        <div class="form-group">
            <label for="cant">Cantidad de alta:</label><br />
			<input type="numeric" class="form-control" id="xcant" name="xcant" onkeypress="return soloNumeros(event)" placeholder="Cantidad a agregar" />
		</div>
        
        <div class="form-group">
        <label for="Marca">Marca:</label>
        <select id="xmarca" name="xmarca" disabled="" class="form-control">
        ...
        <select id="xmodelo" name="xmodelo" disabled="" class="form-control">
            <option value="0">SELECCIONE</option>
            <?php
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" onkeypress="return soloLetras(event)" maxlength="300" name="nota" id="nota"  placeholder="Nota"></textarea>
		</div>
        
        <div class="form-group">
        <label for="Proveedor">Proveedor:</label>
        <select id="xproveedor" name="xproveedor" class="form-control">
            <option value="0">SELECCIONE</option>
            <?php
